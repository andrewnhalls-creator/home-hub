  ...
  return (
    <div className="flex flex-col gap-4" role="status" aria-label="Cargando">
      ...
      <div className="grid grid-cols-2 gap-2">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="flex items-center gap-2.5 rounded-xl border border-border/70 bg-card px-3 py-3 shadow-[var(--shadow-card)]">
      ...
      {/* Content rows */}
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="flex items-center gap-3 rounded-2xl border border-border/70 bg-card p-4 shadow-[var(--shadow-card)]">

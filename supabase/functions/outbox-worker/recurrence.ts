...
/**
 * First occurrence of a date-based recurring rule on or after `fromDate`,
 * skipping any dates in `exceptions`. Returns null if none is found within
 * the search guard (defensive; cannot practically happen with real rules).
 */
export function nextOccurrenceOnOrAfter(
  ...
  const anchor = anchorDay ?? parseInt(startDate.slice(8, 10), 10);
  let cursor = startDate;
  for (let guard = 0; guard < 1000; guard++) {
    if (cursor >= fromDate && !exceptions?.has(cursor)) return cursor;
    cursor = advanceDateOnly(cursor, frequency, anchor);
  }
  return null;
}

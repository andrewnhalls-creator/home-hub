      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
          <select
            ...
            className={cn(
              "min-h-[44px] w-full appearance-none rounded-[var(--radius-xl)] border border-border bg-card px-3 py-2.5 pr-10 text-sm text-brown transition-[border-color,box-shadow] duration-150 focus:outline-none focus:border-terracotta/70 focus:ring-1 focus:ring-terracotta/50 disabled:cursor-not-allowed disabled:opacity-40 [color-scheme:light]",
          ...
          >
            ...
            {options.map((option) => (
              <option key={option.value} value={option.value} className="bg-card text-brown">
                {option.label}
              </option>
            ))}
          </select>
